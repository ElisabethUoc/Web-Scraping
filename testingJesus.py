...
time.sleep(0.7)  # Allow 2 seconds for the web page to open
scroll_pause_time = 0.3 # You can set your own pause time. My laptop is a bit slow so I use 1 sec
...
for i in range(len(listings)):
    for feature in RULES_SEARCH_PAGE:
        try:
            mixed_row.append(extract_element(listings[i], RULES_SEARCH_PAGE[feature]))
        except:
            mixed_row.append("empty")

list_of_rows = [mixed_row[i:i+7] for i in range(0, len(mixed_row), 7)]
